...
public class SaTokenConfigure {
    ...
    /**
     * Sa-Token 参数配置：<a href="https://sa-token.cc">参考文档</a>
     * 此配置会与 application.properties 中的配置合并 （代码配置优先）
     */
    @Autowired
    public void configSaToken(SaTokenConfig config) {
        config.setTokenName("satoken");         // token 名称（同时也是 cookie 名称）
        config.setTimeout(30L * 24 * 60 * 60);   // token 有效期（单位：秒），默认30天，"-1"代表永不过期
        ...
        config.setIsPrint(false);               // 关闭控制台banner
    }
